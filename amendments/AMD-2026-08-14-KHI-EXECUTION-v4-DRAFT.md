# Amendment Package — KHI Family Execution, v4 CONSOLIDATED (A20–A31)

**Status: DRAFT v4 — FREEZE CANDIDATE.** Supersedes `AMD-2026-08-14-KHI-EXECUTION-v3-DRAFT.md` (retained, append-only), which
consolidated the v2 main package and A28-v2. v4 applies exactly the two
blocking fixes from confirmation pass
`REVIEW-2026-08-14-GPT-5.6-Sol-04-CONFIRMATION.md` — [P3-V5] JCS hash
constructions + null_id definition, and [P3-V13] `UNSCOREABLE_N2_SUPPORT` —
both adopted verbatim, plus one flagged consistency edit: the A28.5
table-ordering hash is written SHA-256(JCS(triple_id)) under the same rule.
No other text differs from v3. Prepared 2026-08-14 after verification pass
...
Freezing requires: (1) external confirmation that the two [P3] fixes and the
flagged A28.5 edit are correctly applied;
...
## Final confirmation instructions (external reviewer)

Verify only: (1) the [P3-V5] fix — the four hash/seed constructions and the
JCS/null_id definition — matches your required text exactly; (2) the
[P3-V13] fix — status vocabulary and precedence — matches your required
text exactly; (3) the flagged A28.5 consistency edit
(SHA-256(JCS(triple_id)) for table ordering) is sound; (4) no other line
differs from the v3 you reviewed. End with CLEAR TO FREEZE or blocking
findings.
