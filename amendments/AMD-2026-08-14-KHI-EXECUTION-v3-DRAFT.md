# Amendment Package — KHI Family Execution, v3 CONSOLIDATED (A20–A31)

**Status: DRAFT v3 — FREEZE CANDIDATE.** Supersedes and consolidates
`AMD-2026-08-14-KHI-EXECUTION-v2-DRAFT.md` and
`AMD-2026-08-14-A28-SCREEN-POSITIVES-v2-DRAFT.md` (both retained, append-only)
into a single self-contained text. Prepared 2026-08-14 after verification pass
...
Freezing requires: (1) external confirmation pass 3 verifying the 14 edits;
(2) operator ratification of R-1 and R-2 below; (3) signed tag
`freeze/amendments-v1` on an exact commit, anchored **before** it gates
anything (GOVERNANCE R4 as amended).
...
3. **Registered scoring extent** [V4]. If |E_raw| ≤ 1,000,000, set E = E_raw.
   ...
   ascending SHA-256(triple_id ∥ "KHI-A20-LOTTERY-20260814") with A20.9
   canonicalization. Thereafter **E means only the registered scoring extent
...
5. **Stage 1 — screening.** Every triple in E scored with **exactly 9,999 N1
   ...
   ties as exceedances, streams seeded SHA-256("KHI-S1" ∥ triple_id ∥
   null_id), Monte Carlo uniformly (no exact-enumeration substitution at
...
6. **Stage 2 — confirmatory, single frozen algorithm** [F4]: per survivor, per
   ...
   SHA-256("KHI-S2" ∥ triple_id ∥ null_id), independent of every Stage-1
...
4. **Panel membership by hash lottery** [F20, V5]: within each frozen stratum,
   ...
   SHA-256(doc_id ∥ "KHI-A22-LOTTERY-20260814") under A20.9 canonicalization;
...
3. **Terminal status** [A28-F10, V13]. Every triple in E receives exactly one
   terminal status from: `S1_ONLY_FAIL_RAW`, `S2_FAIL_BOTH_NULLS`,
   `S2_FAIL_N1`, `S2_FAIL_N2`, `S2_FAIL_HOLM`, `S2_FAIL_DECOY_RANK`,
   `UNSCOREABLE_N1_SUPPORT`, `UNSCOREABLE_DECOY_STRATUM`, `UNSCOREABLE_QC`,
   `PASS_ADJUDICATION`. **Precedence (frozen; first matching state wins)
   [V13]: Stage-1 raw failure → N1-support → decoy-stratum → QC → both
   Stage-2 raw nulls fail → N1 only → N2 only → Holm → decoy-rank → pass.**
   Only `S2_FAIL_*` statuses are SPNA-eligible.
...
5. **Mandatory exhaustive, non-salient reporting** [A28-F5, F7, F12]. All
   ...
   renderings use deterministic SHA-256(triple_id) order, never score or
...
## Confirmation pass 3 instructions (external reviewer)

Verify only: (1) each of verification findings V1, V3–V14 is implemented by
the clause cited above, with [V1, V3, V5, V7, V8, V9, V10] adopted verbatim;
(2) GOVERNANCE.md as edited resolves V11 (canonical-record sentence, R4,
freeze steps); (3) no edit introduced a new inconsistency. End with CLEAR TO
FREEZE or blocking findings.
